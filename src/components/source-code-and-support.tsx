...
export const SourceCodeAndSupport = (props: { h?: boolean }) => {
	return (
		<div class={`flex ${props.h ? 'flex-row gap-1.5 md:gap-3' : 'flex-col gap-1 md:gap-2'} items-center justify-center`}>
			<p class="group text-xs">
			...
			>
				<div class="flex flex-col gap-2 mt-3 mb-1 min-w-40">
					<Button as="a" href="https://buymeacoffee.com/flamrdevs" target="_blank" rel="noopener noreferrer" size="sm" class="group justify-start gap-2 pl-2 pr-3 w-full">
						...
						<span class="grow mr-4">Buy Me a Coffee</span>
						<LucideIcon i={ExternalLink} class="size-3.5 opacity-0 group-hover:opacity-100" />
					</Button>
					<Button as="a" href="https://paypal.me/flamrdevs" target="_blank" rel="noopener noreferrer" size="sm" class="group justify-start gap-2 pl-2 pr-3 w-full">
						<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" class="size-4 fill-current">
							<title>PayPal</title>
						...
						<span class="grow mr-4">PayPal</span>
						<LucideIcon i={ExternalLink} class="size-3.5 opacity-0 group-hover:opacity-100" />
					</Button>
				</div>
			</Popover>
