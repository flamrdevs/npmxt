...
export async function GET(event: SolidJS.Start.Server.APIEvent) {
	try {
		...
		return withCache(parseCachedPackageName(event.params['name']), async (validPackageName) => chart.m((await fetchPackageDownloadsRangeLast(validPackageName, 'month')).downloads));
	} catch (error) {
