...
export async function GET(event: SolidJS.Start.Server.APIEvent) {
	try {
		...
		return withCache(parseCachedPackageName(event.params['name']), async (validPackageName) => chart.y((await fetchPackageDownloadsRangeLast(validPackageName, 'year')).downloads));
	} catch (error) {
