...
const packageAction = action(async (formData: FormData) => {
	...
	if (pkg) return redirect(`/package/${pkg}`);
}, 'package-action');
const downloadsAction = action(async (formData: FormData) => {
	const pkg = formData.get('pkg');
	if (pkg) return redirect(`/downloads/${pkg}`);
}, 'downloads-action');
			...
			<div>
				<div class="relative isolate px-5 md:px-6 lg:px-8 pt-11 md:pt-14">
					...
					<div class="mx-auto py-20 sm:py-32 lg:py-40">
						...
						<div class="mt-8 md:mt-9 lg:mt-10">
							<SourceCodeAndSupport />
						</div>
					</div>
